...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        """
        ########################################
        for layer_name, params in named_parameters:
            weight = 'weight' in layer_name
            bn = 'bn' in layer_name
            if weight and params.requires_grad and ("bias" not in layer_name) and ("bn" not in layer_name):
                mean_grad = params.grad.abs().mean().item()
                all_grads.append(mean_grad)
                cleaned_layer_name = layer_name.replace('layer_dict.', '').replace('.weight', '').replace('.','_')
                layers.append(cleaned_layer_name)
